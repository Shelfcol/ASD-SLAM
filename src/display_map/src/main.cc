...
int main(int argc, char* argv[]){
    ...
    std::string res_root="/home/gxf/slam/ASD-SLAM/DataSave/chamo.map";//argv[1];
    ...
    for(int i=0; i<map.frames.size(); i++){
        ...
        for(int j=0; j<map.frames[i]->obss.size(); j++){
            if(map.frames[i]->obss[j]!=nullptr){
                ...
                line_segment.scale = 0.03;
                line_segment.alpha = 0.7;

                line_segment.color.red = 0;
                line_segment.color.green = 0;
		...
		average_err = average_err + t_proj_err/mp_count;
		std::cout<<t_proj_err/mp_count<<std::endl;
	...
	std::cout<<"average_reproj_err:"<<average_err / map.frames.size()<<std::endl;
    ros::spin();
